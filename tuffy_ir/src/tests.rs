...
use crate::instruction::{ICmpOp, Op, Origin};
...
fn display_add_function() {
    ...
    assert_eq!(
        ...
        "func @add(int, int) -> int {\n\
         \x20\x20region function {\n\
         \x20\x20\x20\x20bb0:\n\
         \x20\x20\x20\x20\x20\x20v0 = param 0\n\
         \x20\x20\x20\x20\x20\x20v1 = param 1\n\
         \x20\x20\x20\x20\x20\x20v2 = add v0, v1\n\
         \x20\x20\x20\x20\x20\x20ret v2\n\
         \x20\x20}\n\
         }"
    );
}
...
fn display_multi_block_branch() {
    let mut func = Function::new("max", vec![Type::Int, Type::Int], Some(Type::Int));
    let mut builder = Builder::new(&mut func);
    ...
    let a = builder.param(0, Type::Int, Origin::synthetic());
    let b = builder.param(1, Type::Int, Origin::synthetic());
    let cmp = builder.icmp(ICmpOp::Sgt, a, b, Origin::synthetic());
    builder.brif(cmp, bb1, vec![], bb2, vec![], Origin::synthetic());

    builder.switch_to_block(bb1);
    builder.ret(Some(a), Origin::synthetic());

    builder.switch_to_block(bb2);
    builder.ret(Some(b), Origin::synthetic());
    builder.exit_region();
    ...
    assert_eq!(
        ...
        "func @max(int, int) -> int {\n\
         ...
         \x20\x20\x20\x20\x20\x20v0 = param 0\n\
         \x20\x20\x20\x20\x20\x20v1 = param 1\n\
         \x20\x20\x20\x20\x20\x20v2 = icmp.sgt v0, v1\n\
         \x20\x20\x20\x20\x20\x20brif v2, bb1, bb2\n\
         \n\
         \x20\x20\x20\x20bb1:\n\
         \x20\x20\x20\x20\x20\x20ret v0\n\
         \n\
         \x20\x20\x20\x20bb2:\n\
         \x20\x20\x20\x20\x20\x20ret v1\n\
         \x20\x20}\n\
         }"
    );
}
...
fn display_nested_loop_region() {
    let mut func = Function::new("factorial", vec![Type::Int], Some(Type::Int));
    let mut builder = Builder::new(&mut func);
    ...
    // bb0: entry
    builder.switch_to_block(bb0);
    let n = builder.param(0, Type::Int, Origin::synthetic());
    let one = builder.iconst(1, Origin::synthetic());
    let init_acc = builder.iconst(1, Origin::synthetic());
    builder.br(bb1, vec![init_acc, one], Origin::synthetic());

    // bb1: loop header with block args
    let acc = builder.add_block_arg(bb1, Type::Int);
    let i = builder.add_block_arg(bb1, Type::Int);
    builder.switch_to_block(bb1);
    let cmp = builder.icmp(ICmpOp::Sle, i, n, Origin::synthetic());
    builder.brif(cmp, bb2, vec![], bb3, vec![], Origin::synthetic());

    // bb2: loop body
    builder.switch_to_block(bb2);
    let new_acc = builder.mul(acc, i, Origin::synthetic());
    let new_i = builder.sub(i, one, Origin::synthetic());
    builder.continue_(vec![new_acc, new_i], Origin::synthetic());

    // bb3: after loop
    builder.switch_to_block(bb3);
    builder.ret(Some(acc), Origin::synthetic());
    ...
    assert_eq!(
        ...
        "func @factorial(int) -> int {\n\
         ...
         \x20\x20\x20\x20bb0:\n\
         \x20\x20\x20\x20\x20\x20v0 = param 0\n\
         \x20\x20\x20\x20\x20\x20v1 = iconst 1\n\
         \x20\x20\x20\x20\x20\x20v2 = iconst 1\n\
         \x20\x20\x20\x20\x20\x20br bb1(v2, v1)\n\
         \n\
         \x20\x20\x20\x20region loop {\n\
         \x20\x20\x20\x20\x20\x20bb1(v4: int, v5: int):\n\
         \x20\x20\x20\x20\x20\x20\x20\x20v6 = icmp.sle v5, v0\n\
         \x20\x20\x20\x20\x20\x20\x20\x20brif v6, bb2, bb3\n\
         \n\
         \x20\x20\x20\x20\x20\x20bb2:\n\
         \x20\x20\x20\x20\x20\x20\x20\x20v8 = mul v4, v5\n\
         \x20\x20\x20\x20\x20\x20\x20\x20v9 = sub v5, v1\n\
         \x20\x20\x20\x20\x20\x20\x20\x20continue v8, v9\n\
         \x20\x20\x20\x20}\n\
         \n\
         \x20\x20\x20\x20bb3:\n\
         \x20\x20\x20\x20\x20\x20ret v4\n\
         \x20\x20}\n\
         }"
    );
}
