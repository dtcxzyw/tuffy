...
For branch lowering, `isel.rs` also does simple layout-aware control-flow shaping:
...
After prologue/epilogue insertion, the backend performs a small target-specific
cleanup that removes unconditional jumps whose target is the immediately
following labeled block before handing the instruction stream to the encoder.
