...
use super::types::*;
use rustc_middle::mir::{self, BinOp, CastKind, Operand, Rvalue, StatementKind};
use rustc_middle::ty;
                    ...
                    if has_deref {
                        if let Some((addr, projected_ty)) = self.translate_place_to_addr(place) {
                                ...
                                && self.builder.is_local_memory_address(val)
                            {
                                ...
                                let store_val =
                                    if matches!(self.builder.value_type(val), Some(Type::Bool)) {
                                ...
                                if matches!(self.builder.value_type(slot), Some(Type::Ptr(_))) {
                                        ...
                                        && !dest_is_ptr_ty
                                    {
                                        ...
                                        if let Some(fat_val) = ref_fat_src {
                                            ...
                                            // address.
                                            let data_ptr =
                                                if self.builder.is_local_memory_address(val) {
                                                    self.builder.load(
                                                        val.into(),
                                                        8,
                                                        Type::Ptr(0),
                                            ...
                                            if let Some(src_base) = src_slot {
                                                    ...
                                                    && self.builder.is_memory_address(val)
                                                {
                                                    ...
                                                    let store_bytes =
                                                        checked_rvalue.unwrap_or(bytes);
                                                    self.current_mem = self
                                                        .builder
                                                        .store(
                                                            val.into(),
                                ...
                                matches!(self.builder.value_type(val), Some(Type::Ptr(_)))
                                    && match rvalue {
                                        ...
                                        Rvalue::Ref(..) | Rvalue::RawPtr(..) => {
                                            let dest_ty = self
                                                .monomorphize(self.mir.local_decls[place.local].ty);
                                            is_fat_ptr(self.tcx, dest_ty)
                                        }
                                        _ => false,
                                    };
